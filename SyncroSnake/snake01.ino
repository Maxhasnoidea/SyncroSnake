...
void displayStartScreen() {
  ...
  while (true) {
    ...
    display.setCursor(25, 20); // Position for "Syncro"
    display.println("Syncro");

    // Display "Snake" below "Syncro"
    display.setCursor(35, 50); // Position for "Snake"
  ...
  // Flash the "Two players" message 5 times
  for (int i = 0; i < 5; i++) {
    display.fillScreen(BLACK); // Clear the screen
    delay(300); // Pause for a moment

    display.setTextColor(RED); // Set text color
    display.setTextSize(1); // Set text size (smaller letters)

    // Display the message
    display.setCursor(5, 60); // Position for the message
    display.println("Two players have to");
    display.setCursor(5, 75); // Position for the second line
    display.println("input the same direction");
    display.setCursor(5, 90); // Position for the third line
    display.println("to make the snake move");
